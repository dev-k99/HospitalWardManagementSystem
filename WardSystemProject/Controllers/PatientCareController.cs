...
namespace WardSystemProject.Controllers
{
    ...
    public class PatientCareController : Controller
    {
        ...
        public IActionResult CreateMedicationAdministration()
        {
            ...
            {
                TempData["ErrorMessage"] = $"An error occurred while preparing the medication administration form: {ex.Message}";
        ...
        public async Task<IActionResult> CreateMedicationAdministration([Bind("PatientId,MedicationId,Dosage,AdministrationMethod,Notes")] MedicationAdministration medAdmin)
        {
            ...
            try
            {
                // Validate medication schedule permissions
                var medication = await _context.Medications.FindAsync(medAdmin.MedicationId);
                if (medication == null)
                {
                    TempData["ErrorMessage"] = "The selected medication could not be found.";
                    return RedirectToAction(nameof(CreateMedicationAdministration));
                }

                if (medication.Schedule > 4 && !isNursingSister)
                {
                    ModelState.AddModelError("MedicationId",
                        "Only Nursing Sisters may administer Schedule 5 or higher medications.");
                }

                if (!ModelState.IsValid)
                {
                    throw new InvalidOperationException("Form validation failed. Please correct the highlighted errors.");
                }

                medAdmin.AdministrationDate = DateTime.Now;
                medAdmin.AdministeredBy = User.Identity?.Name ?? "Unknown User";
            ...
            {
                TempData["ErrorMessage"] = $"A database error occurred while saving: {dbEx.Message}";
            }
            catch (Exception ex)
            {
            ...
            // Repopulate dropdowns if we return to the form
            var meds = isNursingSister
                ? _context.Medications.Where(m => m.IsActive)
                : _context.Medications.Where(m => m.IsActive && m.Schedule <= 4);

            ViewBag.PatientId = new SelectList(
                _context.Patients.Where(p => p.IsActive && p.PatientStatus == "Admitted"),
                "Id", "FullName", medAdmin.PatientId);

            ViewBag.MedicationId = new SelectList(meds, "Id", "Name", medAdmin.MedicationId);
            ViewBag.IsNursingSister = isNursingSister;
